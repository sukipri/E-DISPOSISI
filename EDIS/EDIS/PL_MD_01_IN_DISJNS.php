<div class="card border-primary mb-3" style="max-width: 40rem;">
  <div class="card-header"><i class="fas fa-plus-circle"></i> ENTRI JENIS DISPOSISI</div>
  <div class="card-body">
    <!--  -->
    <form method="post">
    ...
    <?PHP if(isset($_GET['UPFLAG01'])){ ?>
      <button class="btn btn-danger btn-sm" name="update_dispjns_01">UPDATE DATA</button>
      <?PHP }else{ ?>
      <button class="btn btn-success btn-sm" name="simpan_dispjns_01">SIMPAN DATA</button>
    <?PHP } ?>
    </form>
    <!--  -->
  </div>
...
<table class="table table-bordered table-sm table-striped" style="max-width:30rem;">
  ...
  <?PHP 
      $pl_disp_vflag01_sw = $CL_Q($CONN01,"$SL idmain_flag_01,flag_kode_01,flag_nama_01 FROM srv_flag_01 WHERE flag_jenis_01='DISP'");
